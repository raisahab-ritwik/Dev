...
public class CountryCodeActivity1 extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		populateCountryList();
		CountryListArrayAdapter adapter = new CountryListArrayAdapter(
				CountryCodeActivity1.this, countryList);
		...
		ArrayAdapter<String> mAutoCompleteAdapter = new ArrayAdapter<String>(
				CountryCodeActivity1.this, android.R.layout.simple_list_item_1,
				getResources().getStringArray(
						R.array.autocomplete_country_names));
		...
		act_country_list.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				returnIntent.putExtra(
						RESULT_CONTRYCODE,
						fetchPhoneCodeFromName(act_country_list.getText()
								.toString().trim(), countryList));
				setResult(RESULT_OK, returnIntent);
				imgs.recycle();
				// recycle images
				finish();
	...
	private void populateCountryList() {
		...
		for (int i = 0; i < countrycodes.length; i++) {
			countryList.add(new Country(countrynames[i], countrycodes[i], imgs
					.getDrawable(i)));
		}

	}
	private String fetchPhoneCodeFromName(String countryName,
		...
		for (int i = 0; i < countryList.size(); i++) {
			...
			countryCodeMap
					.put(countryNamefromMap, countryList.get(i).getCode());
